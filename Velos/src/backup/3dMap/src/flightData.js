...
export const VERTIPORTS = {
  ...
  WHITEFIELD: {
    name: "Whitefield Vertiport",
    longitude: 77.7499,
    latitude: 12.9698,
    groundHeight: 895,
  },
  HEBBAL: {
    name: "Hebbal Vertiport",
    longitude: 77.5917,
    latitude: 13.0358,
    groundHeight: 905,
  },
  KORAMANGALA: {
    name: "Koramangala Vertiport",
    longitude: 77.6245,
    latitude: 12.9352,
    groundHeight: 900,
  },
...
function buildCurvedRoute(from, to, { curveFactor = 0.22, curveDir = 1 } = {}) {
  const points = [];
  ...
  for (let i = 1; i <= CRUISE_STEPS; i++) {
    ...
    const mt = 1 - t;
    const lon =
      mt * mt * from.longitude + 2 * mt * t * ctrlLon + t * t * to.longitude;
    const lat =
      mt * mt * from.latitude + 2 * mt * t * ctrlLat + t * t * to.latitude;
...
export function getPhaseAtIndex(index, total) {
