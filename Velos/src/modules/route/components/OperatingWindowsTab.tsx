          ...
          <div key={w.windowId} style={{ ...card, marginBottom: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                ...
                <span style={pill('var(--app-text-muted)', 'var(--app-border-subtle)')}>{w.timezoneRef || '—'}</span>
            ...
            <div style={{ overflowX: 'auto', marginTop: 12 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '60px repeat(24, 1fr)', gap: 2 }}>
                ...
                {Array.from({ length: 24 }).map((_, h) => (
                  <div
                    key={h}
                    style={{ fontSize: 9, color: 'var(--app-text-faint)', textAlign: 'center', fontWeight: 700 }}
                  >
...
function FragmentRow({
  ...
  return (
    <>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--app-text)', padding: '4px 0' }}>{label}</div>
      {Array.from({ length: 24 }).map((_, h) => {
        ...
        return (
          <div
            ...
            style={{
              height: 18,
              background: within ? 'var(--app-primary)' : 'var(--app-border-subtle)',
              borderRadius: 3,
...
function Cell({ label, value }: { label: string; value?: string }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 14, color: 'var(--app-text)', fontWeight: 700 }}>{value || '—'}</div>
    </div>
  );
}
