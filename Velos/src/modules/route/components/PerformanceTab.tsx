...
export default function PerformanceTab({ routeId }: { routeId: string }) {
  ...
  return (
    <div>
        ...
        <div key={p.profileId} style={{ ...card, marginBottom: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              ...
              {p.noiseClass && <span style={pill('var(--app-text-muted)', 'var(--app-border-subtle)')}>Noise {p.noiseClass}</span>}
      ...
      {list.length === 0 && (
        <div style={{ padding: 16, color: 'var(--app-text-faint)', fontSize: 13 }}>No performance profiles.</div>
      )}
    </div>
  );
}
