        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 4 }}>
            {[2, 4, 8, 12].map((h) => (
              <button
                key={h}
                style={{ ...btn, padding: '6px 10px', background: windowHours === h ? 'var(--app-primary-subtle)' : 'var(--app-surface)' }}
      ...
      <div style={card}>
        ...
        >
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8 }}>
            <Legend color="var(--app-primary)" label="Hard Lock" />
            <Legend color="var(--app-text-faint)" label="Soft Lock" />
            <Legend color="#EF4444" label="Conflict" />
            <Legend color="var(--app-border)" label="Cancelled" />
          </div>
        </div>
        <ReservationGantt
...
function Legend({ color, label }: { color: string; label: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 11, color: 'var(--app-text-muted)' }}>
      <span
        style={{
          width: 12,
          height: 12,
