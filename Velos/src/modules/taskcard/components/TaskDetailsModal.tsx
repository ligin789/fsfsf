        ...
        <div className="tc-modal__header">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6, flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span
                className="tc-pill"
                style={{ color: template.accent, background: '#F1F5F9' }}
              >
            ...
            return (
              <button
                ...
                style={{
                  padding: '6px 12px',
                  borderRadius: 999,
                  border: `1px solid ${isActive ? STATUS_COLOR[s] : '#CBD5E1'}`,
                  background: isActive ? STATUS_COLOR[s] : '#FFFFFF',
...
function Meta({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <div style={{ fontSize: 13, color: '#0F172A', fontWeight: 500 }}>{children}</div>
    </div>
  );
}
