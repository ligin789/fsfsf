...
export function ChecklistBody({ task }: { task: ChecklistTask }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--app-text-muted)' }}>
        <span>
          {p.done} / {p.total} complete
        </span>
        <span style={{ fontWeight: 600, color: 'var(--app-text)' }}>{p.pct}%</span>
      </div>
      <div style={{ height: 6, borderRadius: 999, background: 'var(--app-border)', overflow: 'hidden' }}>
        <div
          style={{
            width: `${p.pct}%`,
            height: '100%',
      ...
      {task.items.map((item) => (
        <label
          ...
          style={{
            ...
            background: item.done ? '#F0FDF4' : 'var(--app-surface)',
            cursor: 'pointer',
          }}
        >
          <input
