...
const STATUS_COLOR: Record<InspectionItemStatus, { fg: string; bg: string; label: string }> = {
  pending: { fg: 'var(--app-text-subtle)', bg: 'var(--app-border-subtle)', label: 'Pending' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div style={{ fontSize: 12, color: 'var(--app-text-muted)' }}>
        <strong style={{ color: 'var(--app-text)' }}>{PHASE_LABEL[task.phase]}</strong> inspection
      </div>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        ...
        <Chip color="var(--app-text-subtle)" bg="var(--app-border-subtle)">
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      <div style={{ fontSize: 12, color: 'var(--app-text-subtle)' }}>
        Phase: <strong style={{ color: 'var(--app-text)' }}>{PHASE_LABEL[task.phase]}</strong>
      </div>
      {task.items.map((item) => {
        ...
        return (
          ...
          >
            <span style={{ flex: 1, fontSize: 13, color: 'var(--app-text)' }}>{item.label}</span>
            <span
              style={{
                fontSize: 11,
                fontWeight: 600,
  ...
  return (
    <button
      ...
      style={{
        padding: '4px 8px',
        borderRadius: 6,
        border: `1px solid ${danger ? '#FCA5A5' : 'var(--app-border)'}`,
        background: 'var(--app-surface)',
        color: danger ? '#B91C1C' : 'var(--app-text)',
        fontSize: 11,
        fontWeight: 600,
        cursor: 'pointer',
      }}
