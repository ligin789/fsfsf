...
export default function VertiportCard({ vertiport, delay = 0 }: VertiportCardProps) {
  ...
  const classColors: Record<string, { from: string; mid: string; to: string }> = {
    ...
    VERTIPORT: { from: "#1D4ED8", mid: "#2563EB", to: "#3B82F6" },
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "20px 20px 16px 20px", marginTop: "4px" }}>
        ...
        >
          {vertiport.vertiportCode} <span style={{ margin: "0 6px", color: "#CBD5E1" }}>|</span> {vertiport.iataCode || "—"}
        </p>
        ...
        >
          <div style={{ fontSize: "0.75rem", color: "#64748B" }}>
            <span style={{ fontWeight: 600, color: "#1E293B" }}>{vertiport.totalFatoCount}</span> FATO
          </div>
          <div style={{ fontSize: "0.75rem", color: "#64748B" }}>
            <span style={{ fontWeight: 600, color: "#1E293B" }}>{vertiport.totalPadCount}</span> Pads
          </div>
          <div style={{ fontSize: "0.75rem", color: "#64748B" }}>
            <span style={{ fontWeight: 600, color: "#1E293B" }}>{vertiport.totalGateCount}</span> Gates
          </div>
        </div>
      </div>
    </motion.div>
